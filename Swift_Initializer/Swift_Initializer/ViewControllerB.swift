...
class SomeClassTwo {
    //这种类型的闭包会创建一个跟属性类型相同的临时变量，然后修改它的值以满足预期的初始状态，然后返回这个临时变量，作为属性的默认值
    
    /**
    注意：如果你使用闭包来初始化属性，请记住在闭包执行的时候，实例的其他部分还没初始化，这意味着你不能再闭包里访问其他属性，即使这些属性有默认值，同样，你也不能使用隐士self属性，或调用任何实例ff
    */
